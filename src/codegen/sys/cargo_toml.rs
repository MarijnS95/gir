...
fn fill_empty(root: &mut Table, env: &Env, crate_name: &str) {
...

    {
        ...
        if let Some(links) = nameutil::shared_libs_to_links(&env.namespaces.main().shared_libs) {
            set_string(package, "links", links);
        }
...
fn fill_in(root: &mut Table, env: &Env) {
...

    {
        ...
        if let Some(lib_name) = ns.package_name.as_ref() {
